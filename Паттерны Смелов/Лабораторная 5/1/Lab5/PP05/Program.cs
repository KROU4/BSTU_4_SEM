...
namespace Lab05
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                IWriter writer = new FileWriter(@"C:\test2.txt");
                IWriter rsa = new DecRSA(writer);
                IWriter hash = new DecSHA512(rsa);
            ...
            {
                IWriter writer = new FileWriter(@"C:\test4.txt");
                IWriter rsa = new DecRSA(writer);
                IWriter hash = new DecMD5(rsa);
            ...
            {
                IWriter writer = new FileWriter(@"C:\test12.txt");
                IWriter rsa = new DecRSA(writer);
                IWriter hash = new DecSHA512(rsa);
                string? result = hash.Save("BBBBCCCCC");
            ...
            {
                IWriter writer = new FileWriter(@"C:\test13.txt");
                IWriter rsa = new DecRSA(writer);
                IWriter hash = new DecMD5(rsa);
                string? result = hash.Save("BBBBCCCCC");
        ...
        static bool testSHA512(string? message, char delimiter = '\uffff')
        {
            ...
            try
            {
                ...
                string xs = message.Substring(0, k1); // сообщение
                string xsbhcs = message.Substring(k1 + 1); //хеш
        ...
        static bool testSHA512_SA(string? message, char delimiter = '\uffff')
        {
            ...
            try
            {
                ...
                string xs = message.Substring(0, k1); // сообщение
                string xsbhcs = message.Substring(k1 + 1, k2 - k1 - 1); // хеш
                string xparam = message.Substring(k2 + 1); // ключ
