...
FROM TEACHER
WHERE EXTRACT(MONTH FROM BIRTHDAY) = EXTRACT(MONTH FROM SYSDATE) + 3
;
...
BEGIN
    BEGIN
        RAISE EX_MYEX; -- Поднятие исключения
    EXCEPTION
        WHEN EX_MYEX THEN -- Обработка исключения внутри вложенного блока
            DBMS_OUTPUT.PUT_LINE('EX_MYEX raised inside inner block');
            DBMS_OUTPUT.PUT_LINE(SQLCODE || ' ' || SQLERRM);
            RETURN;
    END;
EXCEPTION
    WHEN EX_MYEX THEN 
        DBMS_OUTPUT.PUT_LINE('EX_MYEX raised in outer block');
        DBMS_OUTPUT.PUT_LINE(SQLCODE || ' ' || SQLERRM);
END;
...
BEGIN
    ...
    IF MAX_SALARY IS NULL THEN
        DBMS_OUTPUT.PUT_LINE('No salaries found.');
    ELSE
